 ...
 * @since 1.7.1
 */
public class AbstractEBusWriteConnector extends AbstractEBusConnector {
	...
	/** run connector in exact but slow mode */
	...
	private void checkSendStatus() {
		...
		if(sendEntry != null) {
			...
			logger.trace("sendEntry is still in use...");
	...
	public void setWriteSlow(boolean slowMode) {
	...
	protected void send(boolean secondTry) throws IOException {
		...
		for (int i = 0; i < dataOutputBuffers.length; i++) {
			...
			readWriteDelay = (System.nanoTime() - lastReceiveTime) / 1000;

			outputStream.write(b);
			outputStream.flush();

			// read byte after each send byte in slow mode
			// in normal only read first byte
			if(isWriteSlow() || i == 0) {
				read = inputStream.read();
				readByte = (byte) (read & 0xFF);
			}
			
			// do arbitration on on first byte only
			if(i == 0) {
				...
				if(b != readByte) {
					...
					if(readWriteDelay > 104) {
						logger.warn("Delay {}us, allowed max. 104us...", readWriteDelay);
					}

					// written and read byte not identical, that's
					// a collision
					if(readByte == EBusTelegram.SYN) {
						logger.warn("eBus collision with SYN detected!");
					} else {
						logger.warn("eBus collision detected! 0x{}", 
		...
		if(dataOutputBuffers[1] == (byte)0xFE) {
			...
			if(!isWriteSlow()) {
				...
				// now reads written send data excluding first byte
